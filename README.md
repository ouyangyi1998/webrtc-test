...
### 🌐 网络稳定性 (v1.1 更新)
- **智能心跳保活**：双端（Web/Android）+ 服务端三重心跳检测，心跳超时（60s）自动清理僵尸连接
- **僵尸 Session 清理**：服务端准确识别并清理无效 Session，解决长时间后台恢复后只有 2 人却无法加入的问题
- **精准重连机制**：
  - **Android**：网络切换时只在 ICE 断开时重连，保留有效 TURN 连接
  - **Web**：区分前后台状态，避免后台无效重连消耗资源
- **延迟发送 peer-left**：服务端 3 秒缓冲，避免网络波动导致的误判断连
- **ICE 自动重启**：检测到 ICE 连接断开自动触发重启流程（最多 3 次）
- **状态同步**：重连后自动同步最新 RTC 状态，无需重新协商

### 🎬 流畅性优化
- **SVC 分层编码**：L1T1/L1T2/L1T3 动态切换，弱网更流畅
- **分辨率自适应**：根据码率自动缩放 (1080p → 720p → 540p → 360p)
- **硬件编码优先**：H.264 High Profile + VP8/VP9 硬件加速
- **鼠标事件节流**：移动事件限制 30fps，减少 50% 消息量

### ⚡ 性能优化
- **画面卡顿检测**：自动检测并请求关键帧
- **QoS 分级降级**：三级网络状况判定 (Critical/Moderate/Mild)
- **FEC 冗余编码**：丢包自动恢复，减少花屏
